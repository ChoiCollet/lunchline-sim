...
const PATH_POINTS = [
  ...
  { x: 310, y: 590, m: "입장 대기 후 아래로 — 두 줄로 갈라지는 지점" }
];
const GREEN_START_IDX = 2, GREEN_END_IDX = 5, RED_IDX = 5, FORK_IDX = 6;
...
const RED_T = GREEN_END_T;                                  // 빨간 입장 대기선(초록선 끝과 동일 지점)
const FORK_T = tAtIndex(MAIN_PATH, FORK_IDX);                // 두 줄로 갈라지는 지점
const RED_MARK = corridorPoint(RED_T);
const DOOR_POINT = corridorPoint(FORK_T);

// 포크(두 줄로 갈라지는 지점) 기준: 왼쪽 줄(짝수반 左)은 위쪽 배식대 그룹, 오른쪽 줄(홀수반 右)은
// 그보다 아래쪽 배식대 그룹으로 — 모두 직각(수직으로 갈라진 뒤 수평으로 꺾임)으로 이동
const FORK_BASE = DOOR_POINT;
const COUNTER_X = 420;           // 배식대 구역이 시작되는 x좌표(모든 배식대 공통)
const FORK_L_Y = 520;            // 왼쪽 줄(짝수반)이 꺾이는 높이
const FORK_R_Y = 590;            // 오른쪽 줄(홀수반)이 꺾이는 높이 — 포크 트렁크 끝과 동일
const FORK_TARGET_L = { x: COUNTER_X, y: FORK_L_Y };
const FORK_TARGET_R = { x: COUNTER_X, y: FORK_R_Y };
// 직각 전용 포크 경로: 세로 트렁크(FORK_BASE→해당 높이)를 타고 오르내린 뒤 수평으로 꺾여 배식대로
function forkPoint(t, side) {
  ...
  const localU = vertLen > 0 || horizLen > 0 ? (d - vertLen) / (horizLen || 1) : 0;
...
const DOOR_ICON_POS = { x: 310, y: 450 };
// 세면대: 문 아이콘을 지나 두 줄로 갈라지기 전, 트렁크 부근에 나란히 배치(아랫라인 밑에만)
const SINK_L = { x: 270, y: 485 };
const SINK_R = { x: 350, y: 485 };
...
class LunchLineSim {
  ...
  drawCorridorScene(ctx, colors) {
    ...
    // 3) 빨간 대기선 통과 후 아래로(회색) — 두 줄로 갈라지는 지점까지
    ...
    // FORK_T(두 줄로 갈라지는 지점)부터 직각으로 꺾여 각 배식대 구역으로(회색)
