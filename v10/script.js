...
const PATH_POINTS = [
  { x: 650, y: 460, m: "계단실(대기열 시작)" },
  { x: 650, y: 420, m: "1-8복도 하단 — 계단 하강 실측 약 8.5m" },
  { x: 500, y: 415, m: "복도 — 왼쪽으로 이동 후 위로" },
  { x: 500, y: 300, m: "복도 위쪽 끝 — 급식실 오른·왼복도 바닥(실측 약 14.5m), 문 앞은 들르지 않고 직행" },
  { x: 450, y: 290, m: "학생 유도 라인 진입 — 초록 유도선 시작(ㄷ자 경로의 아래쪽 변)" },
  { x: 280, y: 285, m: "초록 유도선 바닥변 끝" },
  { x: 280, y: 40,  m: "유도선 꺾임 — ㄷ자 위쪽 코너(유턴)" },
  { x: 420, y: 35,  m: "초록 유도선 위쪽 변 끝" },
  { x: 420, y: 270, m: "복도 되돌아가기 시작 — 학생유도라인 실측 총 약 11.8m 구간 끝" },
  { x: 460, y: 295, m: "복도 되돌아가기 끝" },
  { x: 495, y: 315, m: "급식실 입출구 왼쪽 벽 — 빨간 입장 대기선" },
  { x: 535, y: 335, m: "입구 문 통과(짝수반 左 · 홀수반 右)" },
  { x: 565, y: 350, m: "방풍실(전실) 진입 — 여기서부터 두 줄로 분기" }
];
const GREEN_START_IDX = 4, GREEN_END_IDX = 8, RED_IDX = 10, FORK_IDX = 11;
...
const FORK_TARGET_L = { x: 500, y: 130 };  // 왼쪽 줄이 향하는 배식대 방향(위로 증설)
const FORK_TARGET_R = { x: 640, y: 130 };  // 오른쪽 줄이 향하는 배식대 방향(위로 증설)
...
class LunchLineSim {
  ...
  drawCorridorScene(ctx, colors) {
    ...
    ctx.fillText("학생 유도 라인(초록선, 약 11.8m)", PATH_POINTS[6].x - 6, PATH_POINTS[6].y - 6);
    ctx.textAlign = "left";
    ctx.fillText("급식실 오른·왼복도(약 14.5m)", PATH_POINTS[3].x + 8, PATH_POINTS[3].y - 6);
    ctx.fillStyle = "#8a94a6";
    ctx.fillText("복도 되돌아가기", PATH_POINTS[8].x + 8, PATH_POINTS[8].y + 4);
    ...
    ctx.fillText("↑ 배식대 늘어나는 방향(2개→4개, 실제 급식실은 세로로 긴 구조)",
      (FORK_TARGET_L.x + FORK_TARGET_R.x) / 2, 18);
    ctx.textAlign = "left";
  ...
  drawCounters(ctx, colors, corridorMode) {
    const counterH = 34;
    if (corridorMode) {
      // 세로형 레이아웃: 실제 급식실이 세로로 긴 구조라 배식대는 위쪽으로 증설됨
      const stationW = 20;
      const L_BASE_X = 450, R_BASE_X = 590, counterBaseY = 130, rowSpacing = 75;
      let li = 0, ri = 0;
      this.counters.forEach((c) => {
        const row = c.line === "L" ? li++ : ri++;
        const startX = c.line === "L" ? L_BASE_X : R_BASE_X;
        const y = counterBaseY - row * rowSpacing;
