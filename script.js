...
// B 전략 배식대 레이아웃 — 개수가 늘어나도 옆으로 잘리지 않도록 아래로만 증설(세로 스택)
const CLASS_COUNTER_X = 430, CLASS_COUNTER_TOP = 150, CLASS_COUNTER_ROW_SPACING = 70;
...
const DESCEND_LEN = 110; // 입장 대기선을 지난 뒤, ㄷ자 분기가 시작되기 전까지 조금 더 내려오는 구간 길이
const PATH_POINTS = [
...
// 포크 구간(대기선 통과 후 ㄷ자 분기)에서는 학생이 L쪽/R쪽으로 한 명씩 번갈아 배정되기 때문에,
// "같은 줄" 안에서만 보면 원래 인덱스를 하나씩 건너뛰며 표시되는 셈이라 실제 간격이 본선(학생 유도
// 라인)의 2배로 보이는 문제가 있었음. FORK_DENSITY_FACTOR로 이 구간의 t 폭을 2배 넓혀(=밀도를 2배로
// 높여) 같은 편끼리의 실제 화면상 간격이 본선과 동일하게 유지되도록 보정함.
const FORK_DENSITY_FACTOR = 2;
const QUEUE_T_MAX = FORK_T + (FORK_PHYS_LEN / MAIN_PATH.total) * FORK_DENSITY_FACTOR;
// 안내선(회색 ㄷ자 라인)과 학생 점 모두, 배식대 아이콘 바로 앞에서 살짝 멈추도록 하는 여유값
// (예전에는 안내선만 이 여유를 적용받고 학생 점은 COUNTER_X까지 그대로 이동해 배식대 아이콘과
// 겹치는 문제가 있었음 — 이제 점의 실제 이동 경로에도 동일하게 적용)
const FORK_GUIDE_GAP = 16;
const FORK_GUIDE_MAX_U = 1 - FORK_GUIDE_GAP / FORK_PHYS_LEN;
function queuePoint(t, sideHint) {
  if (t <= FORK_T) return corridorPoint(t);
  const u = clamp((t - FORK_T) / (QUEUE_T_MAX - FORK_T), 0, 1);
  return forkPoint(u * FORK_GUIDE_MAX_U, sideHint);
}
// 세면대: ㄷ자의 "아랫팔"(FORK_R_Y) 선 위가 아니라 그보다 살짝 아래쪽에 배치(회색 길과 겹치지 않도록)
const SINK_L = { x: FORK_BASE.x + 40, y: FORK_R_Y + 18 };
const SINK_R = { x: FORK_BASE.x + 90, y: FORK_R_Y + 18 };

// 배식대: L/R로 좌우 분리하지 않고 한 컬럼(같은 x=COUNTER_X)에서 위로만 계속 증설됨(2x2 금지)
const COUNTER_BASE_Y = 530;   // 맨 아래(첫 번째) 배식대 줄의 y좌표(회색 하강 구간이 길어진 만큼 함께 하향)
...
class LunchLineSim {
  ...
  drawCorridorScene(ctx, colors) {
    ...
    ["L", "R"].forEach(side => {
      ctx.beginPath();
      for (let i = 0; i <= 12; i++) {
        const u = (i / 12) * FORK_GUIDE_MAX_U; // 배식대 아이콘 바로 앞에서 살짝 멈춰 침범하지 않게 함
    ...
    // ㄷ자 양쪽 세로선(x=60 / x=310) 사이 빈 공간에 배치해 선과 글씨가 겹치지 않게 함
    ctx.fillStyle = "#2f9e6e"; ctx.font = "12px sans-serif";
    ctx.fillText(`퇴장 완료: ${this.stats.served} / ${this.totalPop}`, 80, 130);

    // 페널티 격리 — "퇴장 완료"와 마찬가지로 ㄷ자 양쪽 세로선 사이 빈 공간에 배치
    ctx.fillStyle = "#334"; ctx.font = "11px sans-serif";
    ctx.fillText(`새치기 격리 중: ${this.penaltyPool.length}명`, 80, 150);
  ...
  drawClassLineScene(ctx, colors) {
    ...
    // 퇴장 완료 / 새치기 격리 중 — 레인·순찰 영역과 겹치지 않도록, 친구 기다림 구역이 빠지면서
    // 비게 된 우측 상단 여백에 배치
    ctx.fillStyle = "#2f9e6e"; ctx.font = "12px sans-serif";
    ctx.fillText(`퇴장 완료: ${this.stats.served} / ${this.totalPop}`, CLASS_COUNTER_X, 26);
    ctx.fillStyle = "#334"; ctx.font = "11px sans-serif";
    ctx.fillText(`새치기 격리 중: ${this.penaltyPool.length}명`, CLASS_COUNTER_X, 44);

    // 친구 기다림 구역 — 배식대 개수가 늘어나며 우측 칸을 다 쓰므로, 줄 서는 공간(레인) 아래
    // 큰 빈 공간으로 이동(넘치면 "+N개 그룹 더"로 표시)
    const fx = 10, fy = 305, boxW = 410, boxH = 430;
    ...
    const cols = Math.max(1, Math.floor((boxW - 18) / 52)), rowH = 34, maxRows = Math.floor((boxH - 14) / rowH);
  ...
  drawCounters(ctx, colors, corridorMode) {
    ...
    // B 전략(반별 두 줄) — 배식대는 원래 있던 자리(430,150)에서 시작해 개수가 늘어나면
    // 옆으로 잘리지 않도록 "아래로만" 증설(세로 스택). L줄 배식대가 먼저, 그 아래로 R줄 배식대가 이어짐
    const counterH = 34;
    const stationW = 30;
    this.counters.forEach((c, i) => {
      const startX = CLASS_COUNTER_X;
      const y = CLASS_COUNTER_TOP + i * CLASS_COUNTER_ROW_SPACING;
      const labelSuffix = this.cfg.strategy === "E" ? "" : `(${c.line})`;
      ctx.fillStyle = "#334"; ctx.font = "10px sans-serif";
      ctx.fillText(`배식대${i + 1}${labelSuffix}`, startX, y - 6);
      c.stations.forEach((st, si) => {
  ...
  drawAutonomous(ctx, corridorMode) {
    ...
    const autoX = corridorMode ? Math.min(COUNTER_X, maxAutoX) : CLASS_COUNTER_X;
    // B 전략: 배식대가 아래로 증설되는 만큼, 자율배식대도 항상 "가장 아래 배식대"의 바로 아래로 따라 이동
    const classAutoY = CLASS_COUNTER_TOP + this.counters.length * CLASS_COUNTER_ROW_SPACING;
    const autoY = corridorMode ? COUNTER_BASE_Y + 72 : classAutoY;
    ctx.fillStyle = "#334"; ctx.font = "11px sans-serif";
