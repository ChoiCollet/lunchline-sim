...
class LunchLineSim {
  ...
  // 음수대·손씻는 GOOTZ 유닛 2대가 나란히 놓여 있음(급식 후 동선)
  drawExitZone(ctx) {
    const ex = 640, ey = 400, w = 130, h = 40;
    ctx.strokeStyle = "#b7c3da"; ctx.setLineDash([3, 2]);
    ctx.strokeRect(ex, ey, w, h);
    ctx.setLineDash([]);
    ctx.fillStyle = "#667085"; ctx.font = "9px sans-serif";
    ctx.fillText("출구 · GOOTZ 음수대/손씻는대", ex, ey - 4);
    for (let i = 0; i < 4; i++) {
      ctx.strokeStyle = "#c3cad4";
      ctx.strokeRect(ex + 6 + i * 30, ey + 8, 22, 22);
    }
  }
  ...
  drawCorridorScene(ctx, colors) {
    ...
    // 학생 점 (고정 간격 기준으로 실제로 화면에 들어오는 만큼만 시각화, 초과분 텍스트)
    const arr = this.corridor;
    const maxVisible = Math.floor(0.93 / QUEUE_SPACING); // 분리지점~계단실 쪽까지 물리적으로 들어갈 수 있는 인원
    ...
    // 실사 반영: 방풍실 앞 분리 지점 = 실제로는 "입구" 문 2개가 나란히 있는 지점
    // (실측 스캔 확인: 입구·출구가 물리적으로 별도 문이며, 출구 쪽엔 음수대·손씻는 GOOTZ 유닛이 있음)
    ctx.fillStyle = "#8a94a6"; ctx.font = "9px sans-serif";
    ctx.fillText("입구 2개(반별 안내판)", CORR.rightX - 30, CORR.topY + 14);
  ...
  drawCounters(ctx, colors, corridorMode) {
    const counterTop = corridorMode ? 66 : 150;
    const counterH = 34, stationW = 30;
    const baseX = corridorMode ? CORR.rightX + 24 : 430;
    let li = 0, ri = 0;
    this.counters.forEach((c) => {
      const totalW = c.stations.length * stationW;
      const row = c.line === "L" ? li++ : ri++;
      const startX = baseX + row * (totalW + 12);
      const y = counterTop + (c.line === "L" ? 0 : 84);
  ...
  drawAutonomous(ctx, corridorMode) {
    const autoX = corridorMode ? CORR.rightX + 24 : 430;
    const autoY = corridorMode ? 250 : 300;
    ctx.fillStyle = "#334"; ctx.font = "11px sans-serif";
