/* ============================================================
   ...
   계단실(2층) → 1-8복도(수직, 실측 8.5m) → 급식실 오른복도·왼복도 바닥을 따라 곧장 서쪽으로
     ...
     (배식대는 실제 급식실이 넓어지는 방향으로 계속 증설되고, 퇴장은 별도 문·음수대(GOOTZ)
     통로 이용 — drawExitZone 참고)
   ============================================================ */
...
const PATH_POINTS = [
  { x: 385, y: 10,  m: "계단실(대기열 시작)" },
  { x: 385, y: 46,  m: "1-8복도 하단 — 계단 하강 실측 약 8.5m" },
  { x: 130, y: 50,  m: "복도 서쪽 끝 — 급식실 오른·왼복도 바닥(실측 약 14.5m), 문 앞은 들르지 않고 직행" },
  { x: 130, y: 128, m: "학생 유도 라인 진입 — 초록 유도선 시작(바닥변, ㄷ자 경로의 아래쪽 변)" },
  { x: 30,  y: 134, m: "초록 유도선 바닥변 서쪽 끝" },
  { x: 30,  y: 94,  m: "유도선 꺾임 — ㄷ자 왼쪽 코너" },
  { x: 130, y: 88,  m: "초록 유도선 상단(ㄷ자 위쪽 변) — 학생유도라인 실측 총 약 11.8m 구간 끝" },
  { x: 200, y: 78,  m: "복도 되돌아가기 — 급식실 왼복도 상단(회색, 약 7.2m)" },
  { x: 210, y: 44,  m: "급식실 입출구 왼쪽 벽 — 빨간 입장 대기선" },
  { x: 270, y: 12,  m: "입구 문 통과(짝수반 左 · 홀수반 右)" },
  { x: 300, y: 8,   m: "방풍실(전실) 진입 — 여기서부터 두 줄로 분기" }
];
const GREEN_START_IDX = 3, GREEN_END_IDX = 6, RED_IDX = 8, FORK_IDX = 9;
...
const FORK_TARGET_L = { x: 392, y: 18 };   // 왼쪽 줄이 향하는 배식대 방향
const FORK_TARGET_R = { x: 392, y: 155 };  // 오른쪽 줄이 향하는 배식대 방향
...
class LunchLineSim {
  ...
  drawExitZone(ctx) {
    const ex = 406, ey = 230, w = 170, h = 38;
  ...
  drawCorridorScene(ctx, colors) {
    ...
    // 구간 라벨 (Archisketch 실측 평면도 기준 구간명) — 각 지점 오른쪽/왼쪽으로 정렬해 서로 겹치지 않게 배치
    ctx.fillStyle = "#8a94a6"; ctx.font = "9px sans-serif"; ctx.textAlign = "right";
    ctx.fillText("계단실", PATH_POINTS[0].x - 6, PATH_POINTS[0].y + 3);
    ctx.fillText("1-8복도(약 8.5m)", PATH_POINTS[1].x - 6, PATH_POINTS[1].y + 4);
    ctx.fillText("복도 되돌아가기", PATH_POINTS[7].x - 6, PATH_POINTS[7].y - 6);
    ctx.textAlign = "left";
    ctx.fillText("급식실 오른·왼복도(약 14.5m)", 55, PATH_POINTS[2].y - 8);
    ctx.fillStyle = "#2f8a5c";
    ctx.fillText("학생 유도 라인(초록선, 실측 약 11.8m)", 5, PATH_POINTS[5].y - 6);
    ctx.fillStyle = "#c14a4a"; ctx.font = "9px sans-serif"; ctx.textAlign = "right";
    ctx.fillText("빨간 입장 대기선", RED_MARK.x - 14, RED_MARK.y - 14);
    ctx.textAlign = "left"; ctx.fillStyle = "#8a94a6";
    ctx.fillText("배식대1", FORK_TARGET_L.x + 6, FORK_TARGET_L.y - 2);
    ctx.fillText("배식대2", FORK_TARGET_R.x + 6, FORK_TARGET_R.y + 2);
  ...
  drawAutonomous(ctx, corridorMode) {
    ...
    const autoY = corridorMode ? 200 : 300;
    ctx.fillStyle = "#334"; ctx.font = "11px sans-serif";
