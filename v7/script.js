/* ============================================================
   ...
   공간 모델(Polycam LiDAR 실측 스캔: 2026-08-21 전체 층 + 2026-08-18 전실/세면대·음수대 구역):
   계단실 쪽(대기열 시작, 수직 복도 약 18m) → 직각으로 꺾임
   → 메인 복도(수평, 교실 여러 개 통과, 약 24m, 페인트 없는 구간) → 초록 줄서기 라인(문 앞 약 5.7m, GLB 텍스처에서 실측 확인)
   → 빨간 정지선(문턱, 잠시 멈춤) → 문 통과, 방풍실 진입 → 세면대 지나 두 줄로 갈라져 각 배식대로
   ============================================================ */
...
function zoneLabel(t) {
  if (t < 0.28) return "계단실 쪽(수직 복도)";
  if (t < GREEN_START_T) return "메인 복도(교실 여러 개 통과)";
  if (t < RED_T) return "초록 줄서기 라인";
  if (t < FORK_T) return "빨간 정지선 통과, 문 진입";
  return "세면대 지나 배식대로";
}

// ---------- 실측(Polycam LiDAR 2026-08-21 전체층 스캔 + 2026-08-18 전실 스캔) 기반 경로 좌표 계산 ----------
// GLB 텍스처에서 바닥의 초록/빨강 페인트 라인을 픽셀 색상으로 직접 추출해 위치를 확인함:
//   - 초록 줄서기 라인: 복도 벽쪽(z≈-6.4)에서 문 쪽으로 실측 약 5.7m 구간 뚜렷하게 도색됨
//   - 빨간 정지선: 문틀 위치(x≈13.2~13.8)에 "ㄴ"자로 표시됨 — 문 앞에서 잠시 멈추는 지점
// t=0(대기열 맨 뒤, 계단실 쪽) -> 계단실 연결 복도(수직, 실측 약 18m) -> 직각으로 꺾임
// -> 메인 복도(수평, 실측 약 24m, 아직 페인트 없는 구간) -> 초록 줄서기 라인(약 5.7m)
// -> 빨간 정지선(문 앞, 잠시 멈춤) -> 문 통과, 방풍실 진입(짧게 꺾임) -> 세면대 지나 두 줄로 배식대
const CORR = { leftX: 80, rightX: 310, topY: 30, bottomY: 150 }; // rightX = 빨간 정지선(문턱) 위치
const GREEN_START_X = 230; // 초록 라인이 시작되는 x좌표(문턱에서 실측 비율만큼 앞)
const FOYER_END = { x: 370, y: 108 }; // 방풍실(전실) 진입 지점, 문 통과 직후
const REAL_LEN_M = { stair: 18, mainUnmarked: 23.9, green: 5.7, foyer: 9 }; // 실측 참고값(m)

function corridorPoint(t) {
  const { leftX, rightX, topY, bottomY } = CORR;
  const stairLen = bottomY - topY;
  const mainLen = rightX - leftX;
  const foyerLen = Math.hypot(FOYER_END.x - rightX, FOYER_END.y - bottomY);
...
// 실측 색상 좌표를 t값으로 환산 (구간 경계 표시용)
const _stairLen = CORR.bottomY - CORR.topY, _mainLen = CORR.rightX - CORR.leftX,
      _foyerLen = Math.hypot(FOYER_END.x - CORR.rightX, FOYER_END.y - CORR.bottomY),
      _total = _stairLen + _mainLen + _foyerLen;
const GREEN_START_T = (_stairLen + (GREEN_START_X - CORR.leftX)) / _total; // 초록 라인 시작
const RED_T = (_stairLen + _mainLen) / _total;                             // 빨간 정지선(문턱)
const FORK_T = Math.min(1, RED_T + 0.04); // 문 통과 후 아주 짧은 간격을 두고 서서히 두 줄로 갈라짐
...
const FORK_TARGET_L = { x: 390, y: 40 };   // 왼쪽 줄이 향하는 문(윗줄 배식대)
const FORK_TARGET_R = { x: 390, y: 120 };  // 오른쪽 줄이 향하는 문(아랫줄 배식대)
...
class LunchLineSim {
  ...
  drawExitZone(ctx) {
    const ex = 406, ey = 210, w = 170, h = 38;
  ...
  drawCorridorScene(ctx, colors) {
    const cfg = this.cfg;
    // 회색 구간: 계단실 ~ 초록 라인 시작 전까지(아직 페인트 없는 복도)
    ctx.strokeStyle = "#d7dde5"; ctx.lineWidth = 15; ctx.lineCap = "round";
    ctx.beginPath();
    for (let i = 0; i <= 30; i++) {
      const t = (i / 30) * GREEN_START_T;
      const p = corridorPoint(t);
      i === 0 ? ctx.moveTo(p.x, p.y) : ctx.lineTo(p.x, p.y);
    }
    ctx.stroke();

    // 초록 구간: 실측 GLB에서 확인된 실제 바닥 페인트 라인(초록 줄서기 라인, GREEN_START_T~RED_T)
    ctx.strokeStyle = "#4caf7d"; ctx.lineWidth = 15; ctx.lineCap = "round";
    ctx.beginPath();
    for (let i = 0; i <= 16; i++) {
      const t = GREEN_START_T + (i / 16) * (RED_T - GREEN_START_T);
      const p = corridorPoint(t);
      i === 0 ? ctx.moveTo(p.x, p.y) : ctx.lineTo(p.x, p.y);
    }
    ctx.stroke();

    // 빨간 정지선(문턱, 실측 GLB에서 "ㄴ"자로 확인된 위치) — 잠시 멈추는 지점
    ctx.save();
    ctx.strokeStyle = "#d64545"; ctx.lineWidth = 4; ctx.lineCap = "round";
    ctx.beginPath();
    ctx.moveTo(RED_MARK.x - 7, RED_MARK.y - 9);
    ctx.lineTo(RED_MARK.x + 7, RED_MARK.y + 9);
    ctx.stroke();
    ctx.restore();

    // 회색 구간: 문 통과 후 방풍실 진입(짧게 꺾임) ~ 포크 시작 전까지
    ctx.strokeStyle = "#d7dde5"; ctx.lineWidth = 13; ctx.lineCap = "round";
    ctx.beginPath();
    for (let i = 0; i <= 10; i++) {
      const t = RED_T + (i / 10) * (FORK_T - RED_T);
      const p = corridorPoint(t);
      i === 0 ? ctx.moveTo(p.x, p.y) : ctx.lineTo(p.x, p.y);
    }
    ctx.stroke();

    // FORK_T 지점부터 세면대를 지나 실제로 두 갈래(왼쪽줄/오른쪽줄)로 갈라지는 구간을 별도로 표시
    ...
    ctx.fillText("메인 복도(교실 여러 개 통과)", CORR.leftX + 40, CORR.bottomY + 16);
    ctx.fillStyle = "#2f8a5c"; ctx.font = "9px sans-serif";
    ctx.fillText("초록 줄서기 라인(실측 약 5.7m)", GREEN_START_X - 10, CORR.bottomY - 20);
    ctx.fillStyle = "#c14a4a";
    ctx.fillText("빨간 정지선", RED_MARK.x - 26, RED_MARK.y - 16);
    ctx.fillStyle = "#8a94a6"; ctx.font = "10px sans-serif";
    ctx.fillText("방풍실 앞(문)", FORK_BASE.x - 30, FORK_BASE.y - 10);
